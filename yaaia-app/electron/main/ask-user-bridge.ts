/**
 * Bridge for send_message(wait_for_answer=true): waits for user reply (via popup or Telegram).
 */

const ASK_USER_TIMEOUT_MS = 60_000;
const ASK_USER_TIMEOUT_MESSAGE = "User did not reply in time. You can ask again up to 3 times";

let resolveAskUser: ((reply: string) => void) | null = null;
let waitingBusId: string | null = null;
let timeoutTimer: ReturnType<typeof setTimeout> | null = null;

export function isWaitingForAskUser(): boolean {
  return resolveAskUser !== null;
}

export function getWaitingAskUserBusId(): string | null {
  return waitingBusId;
}

export interface WaitForUserReplyOptions {
  timeoutMs?: number;
  onTimeout?: () => void;
  busId?: string;
}

export function waitForUserReply(options?: WaitForUserReplyOptions): Promise<string> {
  const timeoutMs = options?.timeoutMs ?? ASK_USER_TIMEOUT_MS;
  const onTimeout = options?.onTimeout;
  const busId = options?.busId ?? null;

  return new Promise<string>((resolve) => {
    const doResolve = (reply: string) => {
      if (timeoutTimer) clearTimeout(timeoutTimer);
      timeoutTimer = null;
      resolveAskUser = null;
      waitingBusId = null;
      resolve(reply);
    };
    resolveAskUser = doResolve;
    waitingBusId = busId;
    timeoutTimer = setTimeout(() => {
      if (resolveAskUser) {
        resolveAskUser = null;
        waitingBusId = null;
        timeoutTimer = null;
        onTimeout?.();
        resolve(ASK_USER_TIMEOUT_MESSAGE);
      }
    }, timeoutMs);
  });
}

/** Deliver reply from popup (root) or from Telegram. When busId is provided, only delivers if waiting for that bus. */
export function deliverUserReply(reply: string, fromBusId?: string): boolean {
  if (!resolveAskUser) return false;
  if (fromBusId != null && waitingBusId !== fromBusId) return false;
  const fn = resolveAskUser;
  resolveAskUser = null;
  waitingBusId = null;
  if (timeoutTimer) {
    clearTimeout(timeoutTimer);
    timeoutTimer = null;
  }
  fn(reply.trim() || "(no message)");
  return true;
}
