...
Voice calls are optional because they require a second MTProto/media stack and Apple MLX speech models:
...
During an active call, incoming speech is recorded from Telegram media frames, transcribed with `mlx-audio`, appended to the same `telegram-<chat_id>` bus, and routed through the agent. Agent replies to that Telegram bus are synthesized with `mlx-audio` TTS and sent back into the call as PCM frames. `YAAIA_CALLS_TEXT_FALLBACK=1` also sends the same reply as Telegram text.
